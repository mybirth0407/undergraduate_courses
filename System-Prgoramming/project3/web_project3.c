				...
				if (index_check == 0){
					...
					if (strncmp(param[0], "/server_cmd", 4) == 0) {
						...
						if ((pid = fork())<0){
							...
							direct = (char*)malloc(strlen(ROOT) + strlen(param[2]) + 2);

							strcpy(direct, ROOT);
							strcat(direct, "/");
							strcat(direct, param[2]);
							//direct[strlen(ROOT)+strlen(param[2])]='\0';
							strcat(direct, "\0");
							...
							if (execlp(direct, param[2], (char*)0) < 0){
								write(fds[1], "execute file fail", strlen("execute file success"));
								printf("execlp error\n");
							...
							close(fds[1]);
							if( (size = read(fds[0], kk, 256)) == 0)
								write(clientfd, "success\n", strlen("success\n"));
							else
								write(clientfd, kk, strlen(kk));
						}
						...
						write(clientfd, "execute file operation success!!\n", strlen("execute file operation success!!\n"));

						fd = open(param[2], O_RDONLY);

						if(fd < 0){

						write(clientfd, "execute file operation failed @@\n", strlen("execute file operation failed @@\n"));
						}
